...
public class SnailPatrolState : BaseState
{
    ...
    {
        if (
            !currentEnemy.isHurt && !currentEnemy.isDead && !currentEnemy.wait && 
            !currentEnemy.anim.GetCurrentAnimatorStateInfo(0).IsName("SnailPreMove") && 
            !currentEnemy.anim.GetCurrentAnimatorStateInfo(0).IsName("SnailRecover")
        )
            currentEnemy.Move();
    }
